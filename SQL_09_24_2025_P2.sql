...
with temp as (
select product_id, extract('year' from purchase_date) as year, count(*) as num_orders
from Orders
group by 1,2
)
...
t1.product_id = t2.product_id and t1.year = t2.year + 1
where t1.num_orders > 2 and  t2.num_orders > 2
