...
with temp as (
    select user_id, purchase_date - (lag(purchase_date) over(partition by user_id order by purchase_date)) as diff_days
    from Purchases
)

select distinct(user_id)
from temp
where diff_days is not null and diff_days < 8  