...
with trans as (
    ...
    select user_id, sum(amt) as amt_used
    from (
...
select u.user_id, u.user_name, u.credit + coalesce(t.amt_used,0) as credit,
case when u.credit + coalesce(t.amt_used,0) < 1 then 'Yes' else 'No' end as credit_limit_breached
from Users u
left join trans t
