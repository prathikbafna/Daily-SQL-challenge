...
with top_category_tb as (

    select customer_id, category, rank() over(partition by customer_id order by cnt desc, dt desc) as rk
    from (
        select customer_id, category, max(transaction_date) as dt, count(*) as cnt
        from Transactions t
        join Products p
...
select customer_id, sum(amount) as total_amount,
        ...
        round( ((count(transaction_id) * 10) + (sum(amount) / 100))::numeric, 2) as loyalty_score   
from Transactions t
join Products p
on p.product_id = t.product_id
group by 1
...
select tr.customer_id, total_amount, transaction_count, unique_categories, avg_transaction_amount,
category as top_category, loyalty_score
from temp_res tr
join top_category_tb tb
on tr.customer_id = tb.customer_id
where tb.rk = 1
order by loyalty_score desc, 1
